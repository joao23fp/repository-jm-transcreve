...
- **Upload interrompido por timeout**: Sistema salva progresso; usuário pode retomar upload. Créditos não são debitados até que transcrição comece.
...
- **FR-002**: Sistema DEVE calcular consumo de créditos estimado (baseado em duração de arquivo lida no navegador, client-side) ANTES de iniciar upload; créditos são bloqueados antes de enviar o arquivo ao storage
- **FR-003**: Sistema DEVE bloquear (reservar) créditos estimados no momento que upload é confirmado
...
- **FR-005**: Sistema DEVE executar até 3 retentativas automáticas em caso de falha — tanto de transcrição (provedor externo) quanto de análise de IA (prompt de contexto); após 3 falhas consecutivas em qualquer etapa, marcar job como Failed e estornar créditos integralmente
- **FR-006**: Sistema DEVE enviar notificação em tempo real (toast + email) quando processamento de cada arquivo conclui
- **FR-007**: Sistema DEVE permitir que usuário continue navegando enquanto processamento ocorre em background; não deve bloquear UI
...
- Processamento assíncrono é gerenciado por fila de jobs com backend em memória/cache
- Notificações via email usam sistema de fila de emails existente
- Usuário sempre mantém conexão ativa durante upload (resumable uploads estão fora de escopo v1)
